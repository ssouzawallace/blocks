"""
generate_robot_with_front_ultrasonic.py — Blender 3.x / 4.x Python script
==========================================================================
Run from Blender's Script Editor (Alt+P / "Run Script").

Generates the two-wheeled robot with color sensors underneath AND a single
ultrasonic distance sensor mounted on the front, then exports it as
Robots/TwoWheelerWithFrontUltrasonic.fbx.

Hierarchy created
-----------------
TwoWheelerWithFrontUltrasonic (root – attach RobotController.cs)
  ├─ Chassis
  ...
  └─ UltrasonicSensor0       ← attach UltrasonicSensorController.cs (faces +Z / forward)
       ├─ Transducer0        (left "eye")
       └─ Transducer1        (right "eye")
"""
...
def build_robot_with_front_ultrasonic():
    clear_scene()
    ...
    root.name = "TwoWheelerWithFrontUltrasonic"
    ...
    # Color sensors (underneath, facing down)
    ...
    # ── Front Ultrasonic Sensor (facing +Y / forward in Blender coords) ──────
    # The sensor body sits on the front face of the chassis.
    # UltrasonicSensorController casts a ray along transform.forward (+Z in
    # Unity), so the exported object should face forward in the scene.
    us_front_y = CHASSIS_D / 2 + 0.010
    us_z = chassis_z

    # Sensor body (rectangular, yellow – typical HC-SR04 look)
    us_body = add_box("UltrasonicSensor0",
                      location=(0, us_front_y, us_z),
                      dimensions=(0.045, 0.010, 0.020), parent=root)
    assign_material(us_body, mats["us_body"])

    # Transducer "eyes" (two cylinders on the face of the sensor)
    t0 = add_cylinder("Transducer0",
                      location=(-0.012, us_front_y + 0.006, us_z),
                      radius=0.007, depth=0.006,
                      rot_x=math.pi / 2, parent=us_body)
    assign_material(t0, mats["us_eye"])

    t1 = add_cylinder("Transducer1",
                      location=(0.012, us_front_y + 0.006, us_z),
                      radius=0.007, depth=0.006,
                      rot_x=math.pi / 2, parent=us_body)
    assign_material(t1, mats["us_eye"])

    print("TwoWheelerWithFrontUltrasonic built successfully.")
    return root
...
if __name__ == "__main__":
    build_robot_with_front_ultrasonic()
    script_dir = os.path.dirname(bpy.data.filepath) or os.getcwd()
    export_fbx(os.path.join(script_dir, "TwoWheelerWithFrontUltrasonic.fbx"))
